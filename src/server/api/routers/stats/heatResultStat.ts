...
const totalTens = async (ctx: Context, input: z.infer<typeof heatResultStatSchema>) => {
  const query = await ctx.prisma.wave.count({ where: { ...waveFilter(input), waveScore: { gte: 10 } } })
