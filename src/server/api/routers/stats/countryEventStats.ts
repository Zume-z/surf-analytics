...
const getCountryEvents = async (ctx: any, input: z.infer<typeof countryStatSchema>) => {
  const query = {
    ...(await totalEvents(ctx, input)),
    ...(await avgWaveRange(ctx, input)),
    ...(await avgWindConditions(ctx, input)),
...
const eventFilter = (input: z.infer<typeof countryStatSchema>) => ({eventStatus: 'COMPLETED', countrySlug: input.countrySlug, tour: { year: input.year, gender: input.gender } })
...
const avgWaveRange = async (ctx: any, input: z.infer<typeof countryStatSchema>) => {
...
const avgWindConditions = async (ctx: any, input: z.infer<typeof countryStatSchema>) => {
  const query = await ctx.prisma.heat.groupBy({ by: ['windConditions'], where: { event: eventFilter(input), NOT: { windConditions: '-' } }, select: { _count: { select: { windConditions: true } } } })
