...
export default function SurferLocation() {
  ...
  useEffect(() => redirectPrevPage(router, { pathname: '/surfers/[surferId]/locations', query: { surferId: surferId } }), [router])

  const subHeaderData = [
    { content: ( <SubHeaderSurfer surfer={tourResultQuery.data?.[0]?.surfer as Surfer | undefined} flagAlignBottom={true} subData={surferYearSpan(tourResultQuery.data)} routePath={{ pathname: '/surfers/[surferId]/career', query: { surferId: surferId } }} /> ), primaryTab: true }, //prettier-ignore
